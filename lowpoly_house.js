import { vec3, mat4, glMatrix } from './node_modules/gl-matrix/esm/index.js';
...
function initWebGL2() {
    ...
    for (let i = 0; i < indices.length; i += 3) {
        ...
        const vertex1 = vec3.fromValues(vertices[index1 * 3], vertices[index1 * 3 + 1], vertices[index1 * 3 + 2]);
        const vertex2 = vec3.fromValues(vertices[index2 * 3], vertices[index2 * 3 + 1], vertices[index2 * 3 + 2]);
        const vertex3 = vec3.fromValues(vertices[index3 * 3], vertices[index3 * 3 + 1], vertices[index3 * 3 + 2]);

        const normal = calculateNormal(vertex1, vertex2, vertex3);

        vertexData.push(vertex1[0], vertex1[1], vertex1[2], normal[0], normal[1], normal[2]);
        vertexData.push(vertex2[0], vertex2[1], vertex2[2], normal[0], normal[1], normal[2]);
    ...
    */
    ...
    gl.vertexAttribPointer(vertexNormalAttributeLocation, 3, gl.FLOAT, false, 6 * Float32Array.BYTES_PER_ELEMENT, 3 * Float32Array.BYTES_PER_ELEMENT);
    ...
    const rotationAxis = [0, 1, 0]; // rotate around the y-axis
